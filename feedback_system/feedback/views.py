...
def faculty(request):
    ...
    if request.method == 'POST':
        if 'add_empty_records' in request.POST: # add rows
            ...
            indices = list(map(int, indices))
            indices.sort(reverse=True)
            objects = Faculty.objects.all().order_by('faculty_code')
            try:
                for i in indices:
                    faculty = objects[i]
    ...
    queryset = Faculty.objects.all().order_by('faculty_code')
...
def course_offered(request):
    ...
    if request.method == 'POST':
        if 'add_empty_records' in request.POST:  # add rows
            ...
            indices = list(map(int, indices))
            indices.sort(reverse=True)
            objects = CourseOffered.objects.filter(subject_code__offered_by=dept)
            try:
                for i in indices:
                    objects[i].delete()
            except ProtectedError as p:
    ...
    return render_to_response('course_offered.html',
                              {'formset': formset, 'countform': countform, 'deleteform': deleteform,
                               'database': myformset(queryset=CourseOffered.objects.filter(subject_code__offered_by=dept)), 'username': request.user.username,
                               'error': error})
...
def course_registration(request):
    ...
    if course_code:
        ...
        courses = CourseOffered.objects.filter(subject_code__offered_by=dept)
...
def create_course_feedback_assignment(request):
    ...
    error = ''
    dept = Faculty.objects.get(faculty_code=request.user.username).home_department
    course_feedback_objects = CourseFeedbackAssignment.objects.values('course_code__course_code__course_code', 'cycle_no', 'start_date', 'end_date', 'course_code__course_code__course_name').distinct()
    deleteform = DeleteForm()
    if request.method == 'POST':
    ...
    return render_to_response('course_feedback_view.html', {
                            'deleteform': deleteform,
                            'form': create_course_feedback_assignment_form(dept),
...
def view_department_feedback(request):
    ...
    for subject in subjects:
        courses_with_feedback += CourseFeedbackAssignment.objects.values('course_code__course_code__course_code','course_code__course_code__course_name','cycle_no__cycle_no', 'course_code__course_code__subject_code','course_code__course_code__faculty_name__faculty_first_name','course_code__course_code__faculty_name__faculty_last_name').filter(course_code__course_code__subject_code=subject.subject_code, is_given=1).distinct()
